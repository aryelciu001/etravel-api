const userAnswer = require('./data');
const name = (data) => {
    const hotelData = data;
    const results = hotelData.data.body.searchResults.results;      // list of results;
    var minDistanceFromCity = parseFloat(results[0].landmarks[0].distance.split(" ")[0]);
    var maxDistanceFromCity = parseFloat(results[0].landmarks[0].distance.split(" ")[0]);
    var minDistanceFromAirport = parseFloat(results[0].landmarks[1].distance.split(" ")[0]);
    var maxDistanceFromAirport = parseFloat(results[0].landmarks[1].distance.split(" ")[0]);

    var hotelList = [];

    for(let i = 0; i < results.length; i++){
        var distanceFromCity = parseFloat(results[i].landmarks[0].distance.split(" ")[0]);
        var distanceFromAirport = parseFloat(results[i].landmarks[1].distance.split(" ")[0]);
        ...
        hotelList.push([results[i].name, distanceFromCity, distanceFromAirport, starRating, userRating]);
    }
    ...
    hotelReccomendation.sort((a,b)=>{return b[1]-a[1]});
    for(let i = 0; i < 10; i++){
...
const calcPoint = (hotel, userAns) => {
    ...
    var hotelPriority = userAns["Answer Profiling Test"]["hotel"];
